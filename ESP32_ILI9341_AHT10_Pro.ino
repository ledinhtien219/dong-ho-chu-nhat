...
String settingsPage(){
  String networks;
  String page;page.reserve(18000);page=R"HTML(<!doctype html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Smart Clock Control</title><style>
...
<section id="weatherActions" class="panel"><div class="box"><div class="title">Cập nhật trực tiếp</div><p class="lead">Dùng GPS của điện thoại và yêu cầu ESP32 tải ngay nhiệt độ, gió, mưa cùng dự báo 7 ngày.</p><button class="btn" id="weatherNow" type="button">CẬP NHẬT THEO VỊ TRÍ ĐIỆN THOẠI</button><div id="weathermsg" class="coord">Chưa gửi yêu cầu cập nhật</div></div></section>
...
function openPanel(id){tabs.forEach(x=>x.classList.toggle('on',x.dataset.p===id));panels.forEach(x=>x.classList.toggle('on',x.id===id||(id==='display'&&x.id==='displayActions')||(id==='weather'&&x.id==='weatherActions')));action.style.display=id==='firmware'?'none':'flex'}
...
locationInput.addEventListener('input',()=>{latInput.value='';lonInput.value='';gpsMessage.textContent='Đã chuyển sang tìm theo tên địa danh.'});
...
const weatherNow=document.getElementById('weatherNow'),weatherMessage=document.getElementById('weathermsg');async function requestWeather(position){try{const data=position?{lat:position.coords.latitude.toFixed(6),lon:position.coords.longitude.toFixed(6),location:locationInput.value}:{};if(position){latInput.value=data.lat;lonInput.value=data.lon}const r=await fetch('/weather-now',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams(data)});weatherMessage.textContent=r.ok?(position?'Đã nhận GPS. ESP32 đang cập nhật thời tiết...':'Đang cập nhật theo vị trí đã lưu...'):'Không thể cập nhật thời tiết.'}catch(e){weatherMessage.textContent='Mất kết nối với ESP32.'}weatherNow.disabled=false}weatherNow.addEventListener('click',()=>{weatherNow.disabled=true;if(!navigator.geolocation){weatherMessage.textContent='Không lấy được GPS, đang dùng vị trí đã lưu...';requestWeather(null);return}weatherMessage.textContent='Đang lấy GPS điện thoại...';navigator.geolocation.getCurrentPosition(p=>requestWeather(p),()=>requestWeather(null),{enableHighAccuracy:true,timeout:15000,maximumAge:10000})});
...
void setupWebServer(){
  ...
  server.on("/weather-now",HTTP_POST,[]{if(WiFi.status()!=WL_CONNECTED){server.send(503,"application/json","{\"ok\":false,\"error\":\"wifi\"}");return;}if(server.hasArg("lat")&&server.hasArg("lon")){float la=server.arg("lat").toFloat(),lo=server.arg("lon").toFloat();if(la<-90||la>90||lo<-180||lo>180){server.send(400,"application/json","{\"ok\":false,\"error\":\"coordinates\"}");return;}configuredLat=la;configuredLon=lo;coordinatesSet=true;String location=server.arg("location");location.trim();if(location.length())weatherLocation=location;prefs.begin("clock",false);prefs.putBool("hascoord",true);prefs.putFloat("lat",la);prefs.putFloat("lon",lo);if(location.length())prefs.putString("loc",location);prefs.end();}forceWeatherUpdate=true;server.send(202,"application/json","{\"ok\":true,\"updating\":true}");});
