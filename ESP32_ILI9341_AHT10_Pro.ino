...
<section id="weatherActions" class="panel"><div class="box"><div class="title">Cập nhật trực tiếp</div><p class="lead">Nút này mở cầu nối HTTPS để điện thoại cấp GPS chính xác, sau đó tự chuyển tọa độ về ESP32 và tải lại dự báo 7 ngày.</p><button class="btn" id="weatherNow" type="button">CẬP NHẬT THEO GPS ĐIỆN THOẠI</button><div id="weathermsg" class="coord">Chưa gửi yêu cầu cập nhật</div></div></section>
...
const weatherNow=document.getElementById('weatherNow'),weatherMessage=document.getElementById('weathermsg');async function requestWeather(position){try{const place=locationInput.value.trim();const data={lat:position.coords.latitude.toFixed(6),lon:position.coords.longitude.toFixed(6),location:place};latInput.value=data.lat;lonInput.value=data.lon;const r=await fetch('/weather-now',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams(data)});weatherMessage.textContent=r.ok?'Đã nhận GPS chính xác. ESP32 đang tải dữ liệu mới...':'Không thể cập nhật thời tiết.'}catch(e){weatherMessage.textContent='Mất kết nối với ESP32.'}weatherNow.disabled=false}weatherNow.addEventListener('click',()=>{weatherNow.disabled=true;if(!window.isSecureContext||!navigator.geolocation){weatherMessage.textContent='Đang mở trang HTTPS để xin quyền GPS...';location.href='https://raw.githack.com/ledinhtien219/dong-ho-chu-nhat/main/gps.html?device='+encodeURIComponent(location.origin);return}weatherMessage.textContent='Đang lấy GPS điện thoại...';navigator.geolocation.getCurrentPosition(p=>requestWeather(p),e=>{weatherMessage.textContent='Không lấy được GPS: '+e.message;weatherNow.disabled=false},{enableHighAccuracy:true,timeout:15000,maximumAge:10000})});
...
void setupWebServer(){
  ...
  server.on("/gps-apply",HTTP_GET,[]{if(!server.hasArg("lat")||!server.hasArg("lon")){server.send(400,"text/plain; charset=utf-8","Thiếu tọa độ GPS");return;}float la=server.arg("lat").toFloat(),lo=server.arg("lon").toFloat();if(la<-90||la>90||lo<-180||lo>180){server.send(400,"text/plain; charset=utf-8","Tọa độ không hợp lệ");return;}configuredLat=la;configuredLon=lo;coordinatesSet=true;prefs.begin("clock",false);prefs.putBool("hascoord",true);prefs.putFloat("lat",la);prefs.putFloat("lon",lo);prefs.end();forceWeatherUpdate=true;server.send(200,"text/html; charset=utf-8","<!doctype html><meta charset='utf-8'><meta name='viewport' content='width=device-width'><meta http-equiv='refresh' content='3;url=/'><body style='font-family:system-ui;text-align:center;padding:18vh 20px'><h2>Đã nhận GPS chính xác</h2><p>ESP32 đang cập nhật thời tiết. Tự quay lại sau 3 giây...</p><a href='/'>Quay lại ngay</a></body>");});
